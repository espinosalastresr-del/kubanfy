...
class RightsRoyaltyService:
    ...
    async def _authorized(self, user_id: UUID, artist_id: UUID) -> None:
        member = await self.session.scalar(
            select(ArtistMember).where(
                ArtistMember.artist_id == artist_id, ArtistMember.user_id == user_id
            )
        )
    ...
    async def set_splits(
        self, *, user_id: UUID, artist_id: UUID, scope_type: str, scope_id: UUID, splits: list[dict]
    ) -> list[CollaboratorSplit]:
        ...
        if scope_type == "track":
            obj = await self.session.scalar(
                select(Track).where(Track.id == scope_id).with_for_update()
            )
            ...
            obj = await self.session.scalar(
                select(Release).where(Release.id == scope_id).with_for_update()
            )
            if obj is None or obj.artist_id != artist_id:
                raise NotFoundError("Release not found")
        if not splits:
            raise ValidationError("At least one collaborator split is required")
        total = sum(int(x.get("share_bps", 0)) for x in splits)
        if total != 10000:
            raise ValidationError("Collaborator shares must total exactly 10000 basis points")
        if any(int(x.get("share_bps", 0)) <= 0 for x in splits):
            raise ValidationError("Shares must be positive")
        artists = [UUID(str(x["artist_id"])) for x in splits]
        if len(artists) != len(set(artists)):
            raise ValidationError("Duplicate collaborator artist")
        for aid in artists:
            a = await self.session.get(Artist, aid)
            if a is None or a.status != "active":
                raise ValidationError("Invalid collaborator artist")
        current = await self.session.scalar(
            select(func.max(CollaboratorSplit.generation)).where(
                CollaboratorSplit.scope_type == scope_type, CollaboratorSplit.scope_id == scope_id
            )
        )
        generation = (current or 0) + 1
        await self.session.execute(
            update(CollaboratorSplit)
            .where(
        ...
        for item in splits:
            created.append(
                CollaboratorSplit(
                    ...
                    scope_id=scope_id,
                    artist_id=UUID(str(item["artist_id"])),
                    role=str(item.get("role", "artist")),
    ...
    async def get_splits(
        self, *, user_id: UUID, artist_id: UUID, scope_type: str, scope_id: UUID
    ) -> list[CollaboratorSplit]:
        await self._authorized(user_id, artist_id)
        scope_type = scope_type.lower()
        if scope_type not in ("track", "release"):
    ...
    async def append_ledger(
        ...
        *,
        ...
        existing = await self.session.scalar(
            select(RoyaltyLedgerEntry).where(RoyaltyLedgerEntry.idempotency_key == idempotency_key)
        )
    ...
    async def create_settlement(
        ...
        *,
        ...
        if (
            ...
            or gross_cents < 0
            or net_cents < 0
            or net_cents > gross_cents
        ):
            raise ValidationError("Invalid settlement period or amounts")
        existing = await self.session.scalar(
            select(RoyaltySettlement).where(RoyaltySettlement.idempotency_key == idempotency_key)
        )
        if existing is not None:
            return existing
        account = await self.ensure_account(artist_id, currency)
    ...
    async def approve_settlement(
        self, *, user_id: UUID, artist_id: UUID, settlement_id: UUID
    ) -> RoyaltySettlement:
