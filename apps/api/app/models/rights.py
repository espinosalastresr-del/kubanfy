...
class CollaboratorSplit(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "artist_id",
            "generation",
            name="uq_collaborator_split_generation",
        ),
    )
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
...
class RoyaltyLedgerEntry(Base):
    ...
    account_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("royalty_accounts.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
...
class RoyaltySettlement(Base):
    ...
    __table_args__ = (
        UniqueConstraint("idempotency_key", name="uq_royalty_settlement_idempotency"),
    )
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    account_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
